drop database if exists cs_jsp_servlet;
...
select employee.*, position.`name` as position_name,division.`name` as division_name, education_degree.`name` as education_name from employee 
left join `position`  on employee.position_id = `position`.id 
left join division on employee.division_id = division.id
left join education_degree on employee.education_degree_id = education_degree.id;
